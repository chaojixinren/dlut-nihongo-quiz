...
const BANKS = [
  { json: 'public/party-question-bank.json', files: [
    { file: '党史题库_按优先级整理.md', groups: ['party-p1', 'party-p2', 'party-p3', 'party-p4'] },
    { file: '党史题库_完整版.md', groups: ['party-single', 'party-multi', 'party-judge'] },
  ]},
  { json: 'public/military-question-bank.json', files: [
    { file: '军理题库_按优先级整理.md', groups: ['military-p1', 'military-p2', 'military-p3', 'military-p4'] },
    { file: '军理题库_整理版.md', groups: ['military-ch1','military-ch2','military-ch3','military-ch4','military-ch5','military-ch6','military-ch7','military-ch8','military-ch9','military-ch10','military-ch11','military-ch12','military-ch13','military-ch14','military-ch15','military-ch16','military-ch17','military-ch18'] },
  ]},
  { json: 'public/history-question-bank.json', files: [
...
fs.writeFileSync('data/processed/within-file-duplicates.json', JSON.stringify(report, null, 2), 'utf-8')
...
for (const [bank, data] of Object.entries(report)) {
  ...
  for (const [file, info] of Object.entries(data.files)) {
    ...
    console.log(`  ${file}: ${info.total}题 → 唯一${info.unique}, 净重复${info.extraCopies} (${pct}%), cluster ${info.clusterCount}`)
  }
}
