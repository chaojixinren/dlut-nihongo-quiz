...
const consShift = {
  'か':'た', 'き':'ち', 'く':'つ', 'け':'て', 'こ':'と',  // k→t
  'た':'さ', 'ち':'し', 'つ':'す', 'て':'せ', 'と':'そ',  // t→s
  'さ':'は', 'し':'ひ', 'す':'ふ', 'せ':'へ', 'そ':'ほ',  // s→h
  'は':'か', 'ひ':'き', 'ふ':'く', 'へ':'け', 'ほ':'こ',  // h→k
  'ま':'な', 'み':'に', 'む':'ぬ', 'め':'ね', 'も':'の',  // m→n
  'な':'ら', 'に':'り', 'ぬ':'る', 'ね':'れ', 'の':'ろ',  // n→r
  'ら':'や', 'り':'ゆ', 'る':'よ',  // r→y (limited)
}
...
for (const f of files) {
  ...
  let mode = null, currentOpts = []

  function flush() {
    if (mode !== 'B' || currentOpts.length === 0) { currentOpts = []; return }
    ...
    if (dupIdxs.length === 0) { currentOpts = []; return }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (t.match(/^##\s+第\d+课/)) { flush(); mode = null; continue }
    if (t.match(/^###\s+\d+\.\s+/)) { flush(); mode = null; continue }
    if (t.match(/^\*\*[AB]\.\s*给假名选汉字/)) { flush(); mode = 'A'; continue }
    if (t.match(/^\*\*[AB]\.\s*给汉字选假名/)) { flush(); mode = 'B'; continue }
